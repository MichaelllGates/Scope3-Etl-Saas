import logging

import numpy as np
import pandas as pd


# Basic logging config (commercial micro-saas demo)
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")


class Scope3ETLCleaner:
    """Scope 3 logistics ETL pre-cleaning engine.

    Focus: normalize dirty unit strings into standard units so downstream steps can
    safely standardize to t / km.

    Notes:
    - `user_mapping_rules` allows tenant-specific overrides.
    - This cleaner intentionally flags unknown units for manual review.
    """

    DEFAULT_MAPPING = {
        "weight": {
            "kg": "kg",
            "kgs": "kg",
            "kilogram": "kg",
            "kilograms": "kg",
            "千克": "kg",
            "公斤": "kg",
            "t": "t",
            "ton": "t",
            "tons": "t",
            "tonne": "t",
            "tonnes": "t",
            "mt": "t",
            "吨": "t",
            "lb": "lbs",
            "lbs": "lbs",
            "pound": "lbs",
            "pounds": "lbs",
            "磅": "lbs",
        },
        "distance": {
            "km": "km",
            "kms": "km",
            "kilometer": "km",
            "kilometers": "km",
            "公里": "km",
            "千米": "km",
            "mile": "mile",
            "miles": "mile",
            "mi": "mile",
            "英里": "mile",
        },
    }

    def __init__(self, user_mapping_rules: dict | None = None):
        self.user_rules = user_mapping_rules or {"weight": {}, "distance": {}}
        self.active_rules = self._compile_rules()
        logging.info("Scope3ETLCleaner initialized. User rules injected.")

    def _compile_rules(self) -> dict:
        compiled = {
            "weight": self.DEFAULT_MAPPING["weight"].copy(),
            "distance": self.DEFAULT_MAPPING["distance"].copy(),
        }

        for category in ("weight", "distance"):
            if category in self.user_rules:
                custom_rules = {
                    str(k).strip().lower(): str(v).strip().lower()
                    for k, v in self.user_rules.get(category, {}).items()
                }
                compiled[category].update(custom_rules)

        return compiled

    def _normalize_unit(self, raw_unit, category: str) -> tuple:
        if pd.isna(raw_unit):
            return np.nan, True

        raw_lower = str(raw_unit).strip().lower()

        if raw_lower in self.active_rules[category]:
            return self.active_rules[category][raw_lower], False

        return raw_unit, True

    def clean_logistics_data(self, df: pd.DataFrame, weight_col: str, distance_col: str) -> pd.DataFrame:
        result_df = df.copy()

        std_w_col = f"Std_{weight_col}"
        std_d_col = f"Std_{distance_col}"
        review_col = "ETL_Review_Flag"

        result_df[std_w_col] = np.nan
        result_df[std_d_col] = np.nan
        result_df[review_col] = "Clean"

        logging.info("Processing DataFrame: %s rows.", len(result_df))

        if weight_col in result_df.columns:
            w_res = result_df[weight_col].apply(lambda x: self._normalize_unit(x, "weight"))
            result_df[std_w_col] = w_res.apply(lambda x: x[0])
            result_df.loc[w_res.apply(lambda x: x[1]), review_col] = "Needs Manual Review"

        if distance_col in result_df.columns:
            d_res = result_df[distance_col].apply(lambda x: self._normalize_unit(x, "distance"))
            result_df[std_d_col] = d_res.apply(lambda x: x[0])
            result_df.loc[d_res.apply(lambda x: x[1]), review_col] = "Needs Manual Review"

        dirty_count = int((result_df[review_col] == "Needs Manual Review").sum())
        logging.info(
            "ETL Complete. Clean rows: %s, Needs Review: %s",
            int(len(result_df) - dirty_count),
            dirty_count,
        )

        return result_df


class Scope3Cleaner(Scope3ETLCleaner):
    """Compatibility wrapper used by the Streamlit app.

    Phase 2 requirement: accept `custom_mapping` coming from Supabase Rule Memory.

    `custom_mapping` is expected as a flat dict: {raw_unit: std_unit}.
    Category inference based on std_unit:
    - weight: t/kg/lbs
    - distance: km/mile
    """

    def __init__(self, custom_mapping: dict | None = None, user_mapping_rules: dict | None = None):
        rules = user_mapping_rules or {"weight": {}, "distance": {}}

        if custom_mapping:
            weight_units = {"t", "kg", "lbs"}
            distance_units = {"km", "mile"}

            for raw_unit, std_unit in dict(custom_mapping).items():
                raw = str(raw_unit).strip()
                std = str(std_unit).strip().lower()

                if not raw or not std:
                    continue
                if std in weight_units:
                    rules.setdefault("weight", {})[raw] = std
                elif std in distance_units:
                    rules.setdefault("distance", {})[raw] = std

        super().__init__(user_mapping_rules=rules)


if __name__ == "__main__":
    user_settings = {
        "weight": {"kilos": "kg"},
        "distance": {"ml": "mile"},
    }

    cleaner = Scope3ETLCleaner(user_mapping_rules=user_settings)

    raw_data = pd.DataFrame(
        {
            "Shipment_ID": ["S01", "S02", "S03", "S04"],
            "Raw_Weight_Unit": ["KGS", "kilos", "tons", "WT_UNKNOWN"],
            "Raw_Distance_Unit": ["KMS", "ml", "miles", "km"],
        }
    )

    cleaned_data = cleaner.clean_logistics_data(
        df=raw_data,
        weight_col="Raw_Weight_Unit",
        distance_col="Raw_Distance_Unit",
    )

    print("\n========= Clean Result =========")
    print(cleaned_data)
    print("===============================\n")
